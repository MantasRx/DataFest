...
reg_april_SOC <- lm(happiness ~ SOC2A + SOC2B + SOC3A + SOC3B + SOC1 + SOC4A + SOC4B + HHSIZE1 + HH_BANNER, data = trainapril)
...
```{r backwards-general}
testmay <- testmay %>%
  filter(PHYS8!= "(77) DON'T KNOW" & ECON8K != "(99) REFUSED" & ECON6A != "(99) REFUSED" & EDUCATION != "(98) SKIPPED ON WEB" & HHSIZE1 != "(98) SKIPPED ON WEB" & HHSIZE1 !=  "(99) REFUSED" & PHYS5 != "(99) REFUSED")

testapril <- testapril %>%
  filter(PHYS8!= "(77) DON'T KNOW" & ECON8K != "(99) REFUSED" & ECON6A != "(99) REFUSED" & EDUCATION != "(98) SKIPPED ON WEB" & HHSIZE1 != "(98) SKIPPED ON WEB" & HHSIZE1 !=  "(99) REFUSED" & SOC4A != "(99) REFUSED" & PHYS9C != "(99) REFUSED" & EDUCATION != "(77) Not sure" & PHYS3C != "(99) REFUSED" & PHYS3H != "(99) REFUSED")

```

```{r backwards-selection ECON, message = FALSE}

reg_back_may_ECON <- step(reg_may_ECON, direction = "backward")
predictions_ECON = predict(reg_back_may_ECON, testmay)
RMSE(testmay$happiness, predictions_ECON)
summary(reg_back_may_ECON)$adj.r.squared
tidy(reg_back_may_ECON)
```

```{r backwards-econ-april}
reg_back_april_ECON <- step(reg_april_ECON, direction = "backward")
predictions_aprilECON = predict(reg_back_april_ECON, testmay)
RMSE(testmay$happiness, predictions_aprilECON)
summary(reg_back_april_ECON)$adj.r.squared
tidy(reg_back_april_ECON)

```

``` {r backwards-selection-SOC, message = FALSE}
# 
# 
reg_back_may_SOC <- step(reg_may_SOC, direction = "backward")
predictions_SOC = predict(reg_back_may_SOC, testmay)
RMSE(testmay$happiness, predictions_SOC)
summary(reg_back_may_SOC)$adj.r.squared
tidy(reg_back_may_SOC)
# 
reg_back_april_SOC <- step(reg_NN_april_SOC, direction = "backward")
predictions_aprilSOC = predict(reg_back_april_SOC, testmay)
RMSE(testmay$happiness, predictions_ECON)
...
```{r backwards-selection-PHYS, message = FALSE}
...
```{r forwards-selection ECON, message = FALSE}


reg_for_may_ECON <- step(reg_may_ECON, direction = "forward")
 
reg_for_april_ECON <- step(reg_april_ECON, direction = "forward")
summary(reg_for_april_ECON)$adj.r.squared
summary(reg_for_may_ECON)$adj.r.squared

```

``` {r forwards-selection-SOC}
# 
# 
reg_for_may_SOC <- step(reg_may_SOC, direction = "forward")
# 
reg_for_april_SOC <- step(reg_NN_april_SOC, direction = "forward")
summary(reg_for_april_SOC)$adj.r.squared
summary(reg_for_may_SOC)$adj.r.squared
# 
```

```{r forwards-selection-PHYS}
reg_for_may_PHYS <- step(reg_may_PHYS, direction = "forward")
summary(reg_for_may_PHYS)$adj.r.squared
# 
reg_for_april_PHYS <- step(reg_april_PHYS, direction = "forward")
summary(reg_for_april_PHYS)$adj.r.squared
```
...
prediction_reg_NN_may_PHYS <- predict(reg_back_may_PHYS, testmay)


# prediction_reg_NN_back_may_ECON <- predict(reg_NN_back_may_ECON, testmay_NN)

prediction_reg_NN_may_ECON <- predict(reg_back_may_ECON, testmay)


# prediction_reg_NN_back_may_SOC <- predict(reg_NN_back_may_SOC, testmay_NN)

prediction_reg_NN_may_SOC <- predict(reg_back_may_SOC, testmay)
...
prediction_reg_april_SOC <- predict(reg_NN_april_SOC, testapril)
